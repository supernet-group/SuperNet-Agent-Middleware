...
async def create_tag(tag_req: Tag, request: Request):
    ...
    return JSONResponse(await TagService.create_tag(tag_req, access_token))
...
async def get_tags(request: Request):
    ...
    access_token = request.headers.get("Authorization")
    return JSONResponse(await TagService.get_tags(access_token))
...
async def bind_tag(binding_tag_req: BindingTag, request: Request):
    ...
    access_token = request.headers.get("Authorization")
    return JSONResponse(await TagService.bind_tag(binding_tag_req, access_token))